...
ParsedTriangleMesh parse_ply(const fs::path &filename, const Matrix4x4 &to_world) {
    ...
    if (vertices->t == tinyply::Type::FLOAT32) {
        ...
        for (size_t i = 0; i < vertices->count; i++) {
            mesh.positions[i] = xform_point(to_world,
                float3{data[3 * i], data[3 * i + 1], data[3 * i + 2]});
        }
    } else if (vertices->t == tinyply::Type::FLOAT64) {
        double *data = (double*)vertices->buffer.get();
        for (size_t i = 0; i < vertices->count; i++) {
            mesh.positions[i] = xform_point(to_world,
                float3{static_cast<float>(data[3 * i]), static_cast<float>(data[3 * i + 1]), static_cast<float>(data[3 * i + 2])});
        }
    }
    if (uvs) {
        ...
        if (uvs->t == tinyply::Type::FLOAT32) {
            ...
            for (size_t i = 0; i < uvs->count; i++) {
                mesh.uvs[i] = float2{data[2 * i], data[2 * i + 1]};
            }
        } else if (uvs->t == tinyply::Type::FLOAT64) {
            double *data = (double*)uvs->buffer.get();
            for (size_t i = 0; i < uvs->count; i++) {
                mesh.uvs[i] = float2{static_cast<float>(data[2 * i]), static_cast<float>(data[2 * i + 1])};
            }
        }
    }
    if (normals) {
    ...
    if (faces->t == tinyply::Type::INT8) {
        ...
        for (size_t i = 0; i < faces->count; i++) {
            mesh.indices[i] = int3{
                static_cast<int>(data[3 * i]), static_cast<int>(data[3 * i + 1]), static_cast<int>(data[3 * i + 2])};
        }
    }
