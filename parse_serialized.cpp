...
std::vector<float3> load_position(ZStream &zs, int num_vertices) {
...
template <typename Precision>
std::vector<float3> load_normal(ZStream &zs, int num_vertices) {
    ...
    for (int i = 0; i < (int)normals.size(); i++) {
        ...
        zs.read(&y, sizeof(Precision));
        zs.read(&z, sizeof(Precision));
        normals[i] = float3{x, y, z};
    }
    return normals;
}

template <typename Precision>
std::vector<float2> load_uv(ZStream &zs, int num_vertices) {
    std::vector<float2> uvs(num_vertices);
...
template <typename Precision>
std::vector<float3> load_color(ZStream &zs, int num_vertices) {
    std::vector<float3> colors(num_vertices);
